...
- Fixed regridding of HEALPix data defined in xarray without explicit geographic
  coordinates. Previously this produced incorrect output; the fix correctly derives
  lat/lon from the grid specification before regridding.

- Added a clear error when 1D data is passed to a geographic 2D plot context without
  a recognised grid specification. Previously this would produce a cryptic Qhull error
...
This version is a release candidate for the upcoming 1.0 release of earthkit.
...
- **Functional API via** ``ekp.geo`` **(and other namespaces)**: A new high-level functional API
  ...
  See the `API layers notebook <../examples/examples/introduction/03-api-layers.ipynb>`_ for a
...
- Removed the ``fiona`` dependency. ``fiona`` is no longer required; installation is
  lighter and there is no change in functionality for users.
