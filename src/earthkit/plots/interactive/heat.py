# src/earthkit/plots/interactive/heat.py
...
# --- Main Heatmap Function (Corrected to match original logic) ---


# In src/earthkit/plots/interactive/heat.py

# ... (keep all other helper functions the same) ...

# --- Main Heatmap Function (with new color logic) ---
# @inputs.sanitise(is_2d=True)
# def heatmap(data_array, *args, style="continuous", bins=None, bin_labels=None, colorscale=None, **kwargs):
#     if not isinstance(data_array, xr.DataArray) or len(data_array.dims) != 2:
#         return []

#     y_dim, x_dim = data_array.dims
#     x_coords, y_coords = data_array.coords[x_dim].values, data_array.coords[y_dim].values
#     z_values = data_array.values

#     if style == "continuous":
#         return go.Heatmap(
#             z=z_values, x=x_coords, y=y_coords,
...
#         if bins is None: bins = [0, 1, 19, 39, 59, 79, 100]
...
#             x=x_coords,
#             y=y_coords,
#             colorscale=discrete_plotly_colorscale, # Use the newly created scale
#             text=hover_text,
