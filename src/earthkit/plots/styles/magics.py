...
def magics_color(color_name: str) -> str | tuple | None:
    ...
    if upper.startswith("RGBA("):
        ...
        parts = color_name[4:-1].split(",")
        try:
            h, s, lightness = float(parts[0]), float(parts[1]), float(parts[2])
...
def to_yaml_dict(
    ...
    # Both shaded and line-only styles use "colors" — for Contour styles this
    # maps directly to the colors= parameter of Contour.__init__.
