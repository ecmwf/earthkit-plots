...
def _transform_coordinates_parallel(transformer, x_tgt, y_tgt, num_chunks=4):
    ...
    # Create chunk ranges — ensure the final chunk always reaches ny so that
    # rows in the remainder (ny % num_chunks) are not silently dropped.
    chunks = [(i * chunk_size, min((i + 1) * chunk_size, ny)) for i in range(num_chunks)]
    if chunks[-1][1] < ny:
        chunks[-1] = (chunks[-1][0], ny)
    ...
    if x_src.ndim == 1 and y_src.ndim == 1 and z_src.ndim == 2:
        ...
        if is_periodic:
            ...
            if gpu_enabled:
                ...
                interp = RegularGridInterpolator((y_src_gpu, x_ext), z_ext, bounds_error=False, fill_value=np.nan)
                pts = xp.column_stack([y_src_reproj.ravel(), x_reproj_norm.ravel()])
        ...
        if method == "nearest":
            ...
            interp = LinearNDInterpolator(src_points, z_src_cpu.flatten(), fill_value=np.nan)
