...
@multi_variable
def box(data, quantiles=None, dim=None, time_frequency=None, how="mean", color="#F74F4F"):
        ...
        for y, p in zip(quantile_values, quantiles):
            traces.append(
                go.Scatter(
                    ...
                    x=[Timestamp(t).to_pydatetime(t) for t in trace_timesteps],
                    mode='markers',
                    marker={'size': 0.00001, 'color': color},
                    hovertemplate=f"P<sub>{p*100:g}%</sub>: %{{y:.2f}}{units}<extra></extra>",
                )
            )
